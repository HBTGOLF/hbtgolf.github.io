...
function initHeroIntro(){
  ...
  // Full-page black hold.
  setTimeout(()=>intro.classList.add('intro-draw-start'),550);

  // Let the hand-drawn logo complete slowly and breathe.
  setTimeout(()=>intro.classList.add('intro-logo-complete'),3550);

  // Fade the full black page away to reveal the video/site.
  setTimeout(()=>{
    intro.classList.add('intro-fading');
    hero.classList.add('hero-video-visible');
  },4300);

  // Bring the header and bottom status in after the video appears.
  setTimeout(()=>{
    hero.classList.add('hero-ui-visible');
    document.body.classList.add('site-ready');
  },5050);

  // Remove the intro layer completely.
  setTimeout(()=>{
    intro.classList.add('intro-hidden');
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initPremiumMotion();initHeroStatusRotation();initHeroIntro();});
};
