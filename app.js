...
function buildTitleRace(){
  ...
  const map={};
  const chPts=id=>championshipAggregatePoints(id)||0;
  ...
  standings.forEach(entry=>{
    ...
    const season=playerSeason2026FromRounds(p.id);
    ...
    if(p.id==='tf'){
      map[p.id]={
        status:'champion',
        label:'2026 CHAMPION',
        blurb:`Tony stayed in the title fight all season and delivered when it mattered most. He won the Championship Front 9, earned ${fmt(chPts(p.id))} points in Orlando, and finished with ${fmt(points)} points to claim the 2026 HBT title by ${fmt(points-standings[1].points)}. His season was defined by steady improvement, a strong summer run, and a championship finish that turned a 50-point deficit into the trophy.`
      };
    }else if(p.id==='cd'){
      map[p.id]={
        status:'runner-up',
        label:'RUNNER-UP',
        blurb:`Corey put together an excellent season and spent most of the year as the player everyone was chasing. He collected ${wins} regular-season wins, carried the points lead into Orlando, added ${fmt(chPts(p.id))} Championship points, and finished just ${fmt(gap)} points from the title. He did almost everything right, which is unfortunately going to make him even more confident next year.`
      };
    }else if(p.id==='am'){
      map[p.id]={
        status:'final',
        label:'3RD PLACE',
        blurb:`Anthony was one of the league's most consistent players from spring through the Championship. He finished with ${fmt(points)} points, added ${fmt(chPts(p.id))} in Orlando, and posted runner-up finishes on both the Back 9 and the Major Result. His season combined low scoring, plenty of wins, and another year near the top of the HBT standings.`
      };
    }else if(p.id==='rg'){
      map[p.id]={
        status:'final',
        label:'4TH PLACE',
        blurb:`Rick saved some of his best golf for the biggest stage. He won the Championship Back 9, won the 18-hole Major Result, and led the entire field with ${fmt(chPts(p.id))} Championship points. He finished the season at ${fmt(points)} points, tied for third on points, and closed 2026 with the strongest Championship performance of anyone in the league.`
      };
    }else if(p.id==='ss'){
      map[p.id]={
        status:'final',
        label:'5TH PLACE',
        blurb:`Scott had a solid 2026 season with several stretches where he was right in the middle of the weekly fight. He finished with ${fmt(points)} points and opened the Championship with a second-place Front 9 before totaling ${fmt(chPts(p.id))} points in Orlando. His driving and steady play gave him plenty of rounds where he could challenge anyone in the field.`
      };
    }else if(p.id==='jb'){
      const mia=missed.length
        ? ` He also had to work around ${missed.length===1?missed[0]:missed.join(', ').replace(/, ([^,]*)$/, ' and $1')} being an MIA ${missed.length===1?'week':'stretch'}, which made the season harder than the final standings show.`
        : '';
      map[p.id]={
        status:'final',
        label:'6TH PLACE',
        blurb:`Josh kept battling all year and finished 2026 with ${fmt(points)} points.${mia} He added ${fmt(chPts(p.id))} points at the Championship and completed both Crooked Cat nines despite a tough scoring day. The finish does not tell the whole story, and he had plenty of good golf and good moments across the season.`
      };
    }else{
      map[p.id]={
        status:'final',
        label:'SEASON COMPLETE',
        blurb:`${p.first} finished the 2026 season with ${fmt(points)} points and another full year of HBT golf in the books.`
      };
    }
  });
...
function leaderboardGapText(points,leaderPoints){
  ...
  if(gap<0.5) return championshipComplete()?'2026 Champion':'In the lead';
...
function home(){
  ...
  const regularRoundIds=Object.keys(D.rounds).filter(id=>/^R(?:[1-9]|1[0-6])$/.test(id));
  const regularSeasonWinLeaders=D.players.map(player=>({
    player,
    wins:regularRoundIds.reduce((total,roundId)=>{
      const result=(D.rounds[roundId]||[]).find(row=>row.id===player.id);
      return total+(Number(result?.fin)===1?1:0);
    },0)
  })).sort((a,b)=>b.wins-a.wins || Number(a.player.pos)-Number(b.player.pos));
  const statWeek=championshipComplete()
    ? {
        player:byId('ss'),
        display:'79%',
        unit:'FAIRWAYS HIT',
        note:'Best driving accuracy in the field at Crooked Cat · 11 of 14 fairways'
      }
    : {
        player:regularSeasonWinLeaders[0].player,
        display:regularSeasonWinLeaders[0].wins,
        unit:'REGULAR SEASON WINS',
        note:'Most wins in the league through Round 16'
      };
...
<div class="hero-video-treatment" aria-hidden="true"></div>
    ...
    <div class="hero-in home-hero-in">
      <div class="hero-title-wrap hero-event-lockup">
        <div class="hero-event-date">2026 Season Complete</div>
        <div class="hero-event-title">${L.first} ${L.last}</div>
        <div class="hero-event-location">Season Champion · ${fmt(standings[0].points)} points</div>
      </div>
      ...
      <div class="hero-status hero-status-clean">
        ...
        <div class="hero-status-rotator">
          <span id="heroStatusText">${L.last} wins · ${fmt(standings[0].points)} pts</span>
        </div>
      </div>
  ...
  <section class="page home-main">
    <section class="premium-section">
      ...
      <p class="leaderboard-title-race-help">Click a player to read their 2026 season recap.</p>
        ...
        ${standings.map(entry=>{
          ...
          return `<div class="leaderboard-entry reveal" data-title-race-entry="${p.id}">
            <div class="leaderboard-row title-race-row"
              ...
              aria-label="Show ${p.first} ${p.last}'s 2026 season recap">
            ...
            <div class="title-race-detail"
              ...
              aria-hidden="true">
              <div class="title-race-detail-kicker">2026 Season Recap</div>
              <p>${race.blurb}</p>
            </div>
          </div>`;
        }).join('')}
      ...
      <div class="tour-editorial-grid">
        ...
        <article class="tour-story stat-week-card">
          <div class="stat-week-copy">
            ...
            <div class="tour-story-name">${statWeek.player.first} ${statWeek.player.last}</div>
            <div class="tour-story-display">${statWeek.display}</div>
            <div class="tour-story-unit">${statWeek.unit}</div>
            <div class="tour-story-note">${statWeek.note}</div>
          </div>
          <div class="stat-week-headshot" aria-hidden="true">${photoBig(statWeek.player)}</div>
        </article>
      ...
      ${completed.map(s=>{
        ...
        return `<section class="card round-scorecard reveal">
          <div class="round-scorecard-header">
            <div class="round-scorecard-header-top">
              <div class="round-scorecard-title-block">
                <div class="eyebrow">${s.id==='MC'?(s.name||'Midseason Classic'):(String(s.id).startsWith('CH-')?(s.name||s.id):s.id)}</div>
...
function initHeroStatusRotation(){
  ...
  const labels=ordered.map((entry,i)=>{
    const p=entry.player;
    if(i===0) return championshipComplete()
      ? `${p.last} · 2026 Champion · ${fmt(entry.points)} pts`
      : `${p.last} leads · ${fmt(entry.points)} pts`;
