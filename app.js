...
function rankedCard(metric,title,format,dir='max'){
  const arr=[...D.players].sort((a,b)=>dir==='max'?b[metric]-a[metric]:a[metric]-b[metric]);
  return `<div class="card statcard reveal stats-table-reveal"><div class="stat-header"><h2>${title}</h2></div>${arr.map((p,i)=>`<div class="statrow"><div class="rank">${i+1}</div>${avatar(p)}<div class="stat-player-name"><b>${p.last}</b></div><div class="value">${format(p[metric])}</div></div>`).join('')}</div>`;
}
function stats(){
  return `<section class="page stats-page"><div class="pagehead stats-pagehead"><div><h1>2026 Stats</h1></div></div>
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initPremiumMotion();initHeroStatusRotation();setupMenu();});
};
