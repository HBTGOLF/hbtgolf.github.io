  ...
  <section class="page home-main">
    ...
    <section class="premium-section reveal around-tour">
      ...
      <div class="tour-editorial-grid">
        <article class="tour-story tour-story-leader">
          <div class="tour-stat-kicker">Tour Leader</div>
          <div class="tour-story-name">${L.first} ${L.last}</div>
          <div class="tour-story-display">${fmt(L.points)}</div>
          <div class="tour-story-unit">POINTS</div>
          <div class="tour-story-note">${fmt(leadGap)} points clear · 15 of 18 events complete</div>
        </article>
        ...
        <article class="tour-story tour-story-week stat-week-card">
        ...
        <div class="tour-round-pair">
          <article class="tour-story round-card best-round-card">
            <div class="tour-stat-kicker gold-text">Best Round</div>
            <div class="tour-story-display">${seasonBestRound.row.gross}</div>
            <div class="tour-story-name">${seasonBestRound.player.first} ${seasonBestRound.player.last}</div>
            <div class="tour-story-note">${seasonBestRound.round} · ${seasonBestRound.row.plus>=0?'+':''}${seasonBestRound.row.plus} to par</div>
          </article>

          <article class="tour-story round-card worst-round-card">
            <div class="tour-stat-kicker">Worst Round</div>
            <div class="tour-story-display">${seasonWorstRound.row.gross}</div>
            <div class="tour-story-name">${seasonWorstRound.player.first} ${seasonWorstRound.player.last}</div>
            <div class="tour-story-note">${seasonWorstRound.round} · +${seasonWorstRound.row.plus} to par</div>
          </article>
        </div>

        ${recentCourseRecord?`<article class="tour-story course-record-story dark-green">
          <div class="course-record-copy">
            <div class="tour-stat-kicker gold-text">Most Recent Course Record</div>
            <div class="course-record-course">${recentCourseRecord.course.name.replace('FOREST PARK - ','Forest Park · ').replace('TAPAWINGO - ','Tapawingo · ')}</div>
            <div class="course-record-line">
              <span class="tour-story-display">${recentCourseRecord.score}</span>
              <span class="course-record-player">${recentCourseRecord.player.first} ${recentCourseRecord.player.last}</span>
            </div>
            <div class="tour-story-note light-note">${recentCourseRecord.date}</div>
          </div>
          <a href="#player/${recentCourseRecord.player.id}" class="course-record-headshot">${avatar(recentCourseRecord.player)}</a>
        </article>`:''}
      </div>
    </section>
...
function render(){
  ...
  if(menuEl) menuEl.setAttribute('aria-expanded','false');
  window.scrollTo(0,0);
}
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initPremiumMotion();initHeroStatusRotation();setupMenu();ensureHeroVideoPlayback();});
};
