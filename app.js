      ...
      <div class="tablebox home-points compact-points" style="margin-top:22px">
        <table>
          ...
          <tbody>
            ...
            ${['R15','R16','CH'].map(r=>`<tr class="future-points-row"><td class="pname">${r}</td>${D.players.map(()=>'<td>—</td>').join('')}</tr>`).join('')}
            <tr class="leadrow points-total-row"><td class="pname">TOT</td>${D.players.map(p=>`<td class="pts">${fmt(p.points)}</td>`).join('')}</tr>
          </tbody>
        </table>
      </div>
    </section>
...
function playerResultRoundLabel(round){
  return round==='R11'?'Midseason Championship':round==='MC'?'R11':round;
}
  ...
  const bestCareerRoundsTable = `
    <div class="card player-best-rounds-card">
      <div class="tablebox player-best-rounds-table">
        <table>
          <thead><tr><th>Rank</th><th>Course</th><th>Date</th><th>Gross</th><th>Diff</th></tr></thead>
          <tbody>
