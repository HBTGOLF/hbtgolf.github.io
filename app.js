...
function home(){
  ...
  const latestRoundId='R14';
  const latestRoundCourse=D.courses.find(c=>c.name===roundCourseMap[latestRoundId]);
  ...
  const latestRoundRating=latestRoundCourse?.nineRating;
...
<div class="hero-video-treatment" aria-hidden="true"></div>
  ...
  <section class="page home-main">
    ...
    <section class="premium-section reveal latest-home">
      <div class="premium-kicker">Latest Event - Round 14</div>
...
function rounds(){
  ...
  // The workbook's MC bucket contains the June 30 Tapawingo Woodlands
  // regular round, while R11 contains the June 20–21 Crescent Farms
  // 18-hole Midseason Championship. Swap their display order so the
  // Scorecards archive follows the actual calendar.
  const mcDisplayIndex=completed.findIndex(s=>s.id==='MC');
  const r11DisplayIndex=completed.findIndex(s=>s.id==='R11');
  if(mcDisplayIndex>-1 && r11DisplayIndex>-1 && r11DisplayIndex<mcDisplayIndex){
    [completed[mcDisplayIndex],completed[r11DisplayIndex]]=
      [completed[r11DisplayIndex],completed[mcDisplayIndex]];
  }
      ...
      ${completed.map(s=>{
        ...
        return `<section class="card round-scorecard reveal">
          <div class="round-scorecard-header">
            <div class="round-scorecard-header-top">
              <div class="round-scorecard-title-block">
                <div class="eyebrow">${s.id==='R11'?'Midseason Championship':s.id==='MC'?'R11':s.id}</div>
  ...
  const renderBlankYear=(year)=>`
    <div class="rounds-list scorecards-list">
      ${Array.from({length:18},(_,i)=>{
        const label=i<16?`R${i+1}`:i===16?'Midseason Championship':'HBT Championship';
...
function records(){
  ...
  return `<section class="page records-page"><div class="pagehead stats-pagehead"><div><div class="eyebrow">Course book</div><h1>Course Records</h1><p class="records-page-note">All-time HBT records and personal bests from the league handicap history.</p></div></div>
  <div class="coursegrid records-grid">${D.courses.map(c=>`<article class="card record-course-card">
    <div class="course-img ${c.imageClass||''}"></div>
    <div class="round-scorecard-header record-card-header">
      <div class="round-scorecard-header-top">
        <div class="round-scorecard-title-block">
          <div class="eyebrow">Course Record</div>
          <h3>${prettyCourse(c.name)}</h3>
          <div class="round-course-meta">
            <span class="round-tee-meta"><span class="tee-color-dot" data-tee="${c.tees||''}" aria-hidden="true"></span><span>${c.tees||'—'}</span></span>
            <span>${typeof c.nineRating==='number'?c.nineRating.toFixed(2):'—'} | ${c.slope||'—'}</span>
          </div>
        </div>
        <span class="badge gold">${c.holders.length>1?'Holders':'Holder'} · ${c.holders.join(' & ')}</span>
      </div>
    </div>
    <div class="record-summary-row">
      <div class="record-summary-block record-summary-metric">
        <div class="kicker">Low Round</div>
        <div class="course-record">${c.record}</div>
      </div>
      <div class="record-summary-block record-summary-copy">
        <div class="kicker">Course Note</div>
        <div class="record-summary-title">All-Time HBT Record</div>
        <div class="caption">Top six personal best rounds on this course.</div>
      </div>
    </div>
    <div class="tablebox course-record-tablebox">
      <table>
        <thead>
          <tr class="green-header"><th>Rank</th><th>Player</th><th>Best</th></tr>
        </thead>
        <tbody>
          ${c.personal.slice(0,6).map((x,i)=>`<tr class="${i===0?'leadrow':''}"><td class="pos">${i+1}</td><td class="player-cell">${byId(x.id).last}</td><td class="pts">${x.score}</td></tr>`).join('')}
        </tbody>
      </table>
    </div>
  </article>`).join('')}</div>
