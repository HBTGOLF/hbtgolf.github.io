...
function handicapLast20(p){
  const rows=playerHistory(p)
    .filter(r=>Number.isFinite(Number(r.gross)) && Number.isFinite(Number(r.diff)))
    .slice(-20);
  const counting=new Set(
    rows.map((r,i)=>({i,diff:Number(r.diff)}))
  ...
  return rows.map((r,i)=>({
    ...
    iso:r.iso,
    course:r.course,
    score:Number(r.gross),
    diff:Number(r.diff),
    handicap:Number.isFinite(Number(r.hcp))?Number(r.hcp):null,
...
function handicapCalculator(p){
  ...
  <div class="tablebox mobile-calculator" style="border:0"><table class="calculator"><thead><tr><th>Rd</th><th>Date</th><th>Course</th><th>Counts</th><th>Score</th><th>SC Diff</th></tr></thead><tbody>
    ${handicapLast20(p).map((r,i)=>`<tr class="${r.counts?'leadrow':''}"><td>${20-i}</td><td>${r.date}</td><td>${shortCourseName(r.course)}</td><td class="counts">${r.counts?'<span class="count-badge">✓</span>':''}</td><td>${r.score}</td><td>${r.diff.toFixed(2)}</td></tr>`).join('')}
  </tbody></table></div></div>`;
}
