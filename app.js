  ...
  standings.forEach(entry=>{
    ...
    if(guarantee){
      ...
      map[p.id]={
        ...
        blurb:`${intro} A clean guaranteed path is to ${guaranteeText}. Do that and the HBT title is his regardless of the other results.`
      };
      return;
    }
    ...
    if(alive){
      ...
      map[p.id]={
        status:'needs-help',
        label:'NEEDS HELP',
        blurb:`${p.first} ${gapText} and is still mathematically alive. Even winning ${titleRaceActionText()} would not make the title automatic, so he needs the leaders to give back points along the way.`
      };
      return;
    }
    ...
    map[p.id]={
      status:'eliminated',
      label:'ELIMINATED',
      blurb:`${p.first} has been mathematically eliminated from the season championship. Even winning ${titleRaceActionText()} would max him out at ${fmt(titleRaceMaxPoints(p.id))} points, and there is no remaining finish combination that gets him the title.`
    };
  });
      ...
      <div class="leaderboard-stack title-race-leaderboard">
        ${standings.map(entry=>{
          ...
          return `<div class="leaderboard-entry reveal" data-title-race-entry="${p.id}">
            <div class="leaderboard-row title-race-row" data-player-link="${p.id}" role="link" tabindex="0" aria-label="Open ${p.first} ${p.last} player page">
              ...
              <div class="leaderboard-player-copy">
                ...
                <button type="button"
                  class="title-race-pill title-race-${race.status}"
                  data-title-race-toggle="${p.id}"
                  aria-expanded="false"
                  aria-controls="title-race-detail-${p.id}">
                  <span>${race.label}</span><span class="title-race-pill-icon" aria-hidden="true">+</span>
                </button>
              </div>
    ...
    <section class="premium-section reveal around-tour">
      ...
      <div class="tour-editorial-grid">
        ...
        <article class="tour-story stat-week-card tour-story-fire fire-feature-card dark-green">
          <a href="#player/${hot.player.id}" class="stat-week-headshot fire-feature-headshot">${photoBig(hot.player)}</a>
...
function initTitleRaceAccordions(){
  ...
  const closeAll=()=>{
    entries.forEach(entry=>{
      ...
      const btn=entry.querySelector('[data-title-race-toggle]');
  ...
  entries.forEach(entry=>{
    const row=entry.querySelector('[data-player-link]');
    const btn=entry.querySelector('[data-title-race-toggle]');
    const detail=entry.querySelector('[data-title-race-detail]');
    if(!row || !btn || !detail) return;

    const openPlayer=()=>{
      const id=row.dataset.playerLink;
      if(id) location.hash=`player/${id}`;
    };

    row.addEventListener('click',e=>{
      if(e.target.closest('[data-title-race-toggle]')) return;
      openPlayer();
    });

    row.addEventListener('keydown',e=>{
      if((e.key==='Enter' || e.key===' ') && !e.target.closest('[data-title-race-toggle]')){
        e.preventDefault();
        openPlayer();
      }
    });
