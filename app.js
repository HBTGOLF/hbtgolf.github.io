...
function rounds(){
  ...
  const prettyCourse=name=>String(name||'Course')
    .replace('FOREST PARK - ','Forest Park · ')
    .replace('TAPAWINGO - ','Tapawingo · ')
    .replace('CRESCENT FARMS - ','Crescent Farms · ')
    .replace('CREVE COEUR GOLF CLUB','Creve Coeur Golf Club');
  ...
  return `<section class="page scorecards-page">
    <div class="pagehead scorecards-pagehead">
      ...
      <p>16 regular season rounds, 1 midseason classic, and 1 HBT championship.</p>
    </div>
...
function initScorecardYearNav(){
  const buttons=[...document.querySelectorAll('[data-scorecard-year]')];
  if(!buttons.length) return;
  const panels=[...document.querySelectorAll('[data-scorecard-panel]')];

  const showYear=(year)=>{
    buttons.forEach(btn=>{
      const active=btn.dataset.scorecardYear===String(year);
      btn.classList.toggle('active',active);
      btn.setAttribute('aria-current',active?'page':'false');
    });
    panels.forEach(panel=>{
      panel.classList.toggle('active',panel.dataset.scorecardPanel===String(year));
    });
  };

  buttons.forEach(btn=>{
    btn.onclick=()=>showYear(btn.dataset.scorecardYear);
  });

  let startX=null;
  const stage=document.querySelector('.scorecard-year-stage');
  if(stage){
    ...
    stage.addEventListener('touchend',e=>{
      ...
      const next=dx<0?Math.min(current+1,buttons.length-1):Math.max(current-1,0);
      buttons[next]?.click();
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initScorecardYearNav();initPremiumMotion();initHeroStatusRotation();setupMenu();ensureHeroVideoPlayback();});
};
