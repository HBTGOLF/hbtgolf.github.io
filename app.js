...
function playerPageContent(id){
  ...
  const seasonCards = `
    <div class="season-card-grid">
      ...
      <div class="card pad dark"><div class="kicker">HBT Handicap</div><div class="metric metric-with-arrow">${p.hcp.toFixed(2)}<span class="season-mini-arrow">${hcpArrow(p)}</span></div><div class="caption">Current index</div></div>
    </div>`;
  ...
  return `<div class="player-hub-content">
    ...
    <div class="profilehero premium-profile reveal">
      ...
      <div class="big-hcp"><span class="profile-hcp-value">${p.hcp.toFixed(2)}</span><span class="profile-hcp-arrow">${hcpArrow(p)}</span><small>Current HBT Handicap</small></div>
    </div>
    <div class="profile-headline-stats reveal">
      ...
      <div class="profile-headline-stat profile-hcp-stat"><b>${p.hcp.toFixed(2)}<span class="mini-hcp-arrow">${hcpArrow(p)}</span></b><span>HBT Handicap</span></div>
    </div>
...
function render(){
  ...
  if(navEl){navEl.classList.remove('open');navEl.classList.remove('is-open');}
  if(menuEl){menuEl.classList.remove('is-open');menuEl.setAttribute('aria-expanded','false');}
  window.scrollTo(0,0);
}
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initScorecardYearNav();initPlayerHeadNav();initPremiumMotion();initHeroStatusRotation();setupMenu();ensureHeroVideoPlayback();});
};
