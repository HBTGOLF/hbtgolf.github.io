    ...
    <section class="premium-section reveal latest-home">
      ...
      <div class="card home-latest-scorecard masters-latest-card" style="margin-top:20px">
        <div class="latest-scorecard-scroll" role="region" aria-label="${latestRoundLabel(latestRoundId)} scorecard" tabindex="0">
          <table class="masters-latest-scorecard" style="--latest-scorecard-min:${Math.max(478,255+(latestHoleCount*24))}px">
            <thead>
              <tr>
                <th class="latest-player-col" scope="col">Player</th>
                ${latestHoleNumbers.map(h=>`<th class="latest-hole-col" scope="col">${h}</th>`).join('')}
                <th class="latest-total-col" scope="col">Total</th>
                <th class="latest-hdp-col" scope="col" title="HBT Handicap">HDP</th>
                <th class="latest-adj-col" scope="col">Adj</th>
                <th class="latest-finish-col" scope="col">Finish</th>
              </tr>
            </thead>
            <tbody>
              ...
              ${latestScorecardRows.map(r=>{
                ...
                return `<tr class="latest-player-row ${Number(r.finish)===1?'latest-winner-row':''}">
                  ...
                  <td class="latest-hdp-col">${r.hdp??'—'}</td>
                  <td class="latest-adj-col latest-score-adj">${r.adj??'—'}</td>
      ...
      <div class="latest-scorecard-legend" aria-label="Golf scorecard scoring marks">
        <span><i class="golf-score-mark golf-score-birdie" aria-hidden="true">3</i> Birdie</span>
        <span><i class="golf-score-mark golf-score-eagle" aria-hidden="true">3</i> Eagle</span>
        <span><i class="golf-score-mark golf-score-bogey" aria-hidden="true">5</i> Bogey</span>
        <span><i class="golf-score-mark golf-score-double-bogey" aria-hidden="true">6</i> Double+</span>
      </div>
