...
function initScorecardYearNav(){
  ...
  const buttons=[...root.querySelectorAll('[data-scorecard-year]')];
  const panels=[...root.querySelectorAll('[data-scorecard-panel]')];
  const stage=root.querySelector('.scorecard-year-stage');
  if(!buttons.length || !panels.length || !stage) return;
  ...
  buttons.forEach(btn=>{
    // Prevent any inherited/global navigation behavior.
    btn.removeAttribute('onclick');
    btn.addEventListener('click',e=>{
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      showYear(Number(btn.dataset.scorecardYear));
    },true);
  });
  ...
  stage.addEventListener('touchend',e=>{
    ...
    // Only treat a clearly horizontal gesture as a year swipe.
    if(Math.abs(dx)<55 || Math.abs(dx)<=Math.abs(dy)*1.25) return;

    const index=years.indexOf(currentYear);
    const nextIndex=dx<0
      ? Math.min(index+1,years.length-1)
      : Math.max(index-1,0);
