...
function initScorecardYearNav(){
  ...
  const yearNav=root.querySelector('.scorecard-year-nav');
  if(!buttons.length || !panels.length || !yearNav) return;

  const years=buttons.map(b=>Number(b.dataset.scorecardYear));
  let currentYear=Number(buttons.find(b=>b.classList.contains('active'))?.dataset.scorecardYear)||2026;
  ...
  buttons.forEach(btn=>{
    ...
    btn.onclick=null;
    btn.addEventListener('click',activateButton,true);
  });

  // Swipe only on the year-selector strip.
  let startX=0,startY=0,tracking=false;
  yearNav.addEventListener('touchstart',e=>{
  ...
  yearNav.addEventListener('touchend',e=>{
    if(!tracking) return;
    tracking=false;
    ...
    const nextIndex=dx<0 ? Math.min(index+1,years.length-1) : Math.max(index-1,0);
    showYear(years[nextIndex]);
  },{passive:true});
