/* HBT matched build v148 — verified through 2026 R15 */
...
function syncDerivedSeasonData(){
  // Recompute values that can be derived from the round ledger so they cannot drift
  // when a new completed round is added to data.js.
  const completedRoundIds=Object.keys(D.rounds||{}).filter(id=>(D.rounds[id]||[]).some(r=>r.gross!=null));
  const statSource=D.seasonStatSource||null;

  D.players.forEach(p=>{
    const played=completedRoundIds
      .map(id=>(D.rounds[id]||[]).find(r=>r.id===p.id && r.gross!=null))
      .filter(Boolean);
    const grosses=played.map(r=>Number(r.gross)).filter(Number.isFinite);
    const plus=played.map(r=>Number(r.plus)).filter(Number.isFinite);

    p.rounds=played.length;
    p.wins=played.filter(r=>Number(r.fin)===1).length;
    p.lowGross=grosses.length?Math.min(...grosses):p.lowGross;
    p.avgGross=grosses.length?grosses.reduce((a,b)=>a+b,0)/grosses.length:p.avgGross;
    p.avgToPar=plus.length?plus.reduce((a,b)=>a+b,0)/plus.length:p.avgToPar;
    p.points=seasonCurrentPoints(p.id);

    // Putting/GIR/driving are based on the same workbook AVERAGEIF method.
    // The baseline captures completed scorecard rows through R13; later rounds
    // are appended in D.seasonStatSource.rounds.
    const base=statSource?.baseline?.[p.id];
    if(base){
      let count=Number(base.count)||0;
      let putts=Number(base.puttsSum)||0;
      let gir=Number(base.girSum)||0;
      let driving=Number(base.drivingSum)||0;
      Object.values(statSource.rounds||{}).forEach(round=>{
        const row=round?.[p.id];
        if(!row) return;
        if(Number.isFinite(Number(row.putts))){putts+=Number(row.putts);gir+=Number(row.gir);driving+=Number(row.driving);count+=1;}
      });
      if(count){
        p.putts=putts/count;
        p.gir=Math.round((gir/count)*10)/10;
        p.driving=Math.round((driving/count)*10)/10;
      }
    }

    const trend=(p.hc?.trend||[]).map(x=>Number(x.value)).filter(v=>Number.isFinite(v)&&v>0);
    if(p.career && trend.length) p.career.lowestHcp=Math.min(...trend);
  });

  const standings=currentSeasonStandings();
  standings.forEach(({player,points,pos})=>{player.points=points;player.pos=pos;});
  D.players.sort((a,b)=>a.pos-b.pos);
}
syncDerivedSeasonData();
...
function titleRaceControlsBlurb(p,gap,leader,guaranteeText){
  const leaderName=leader.player.first;
  if(p.id==='tf'){
    return `Tony has the wheel and the lead. The assignment is beautifully simple: ${guaranteeText}. Pull that off and there is no scoreboard watching, no tiebreaker drama, and no one else gets a vote — the HBT title is his.`;
  }
  if(p.id==='cd'){
    return `Corey is an extremely annoying ${fmt(gap)} points behind ${leaderName}, hanging around the top of the standings like a three-putt that refuses to go away. Even worse, the math is on his side: ${guaranteeText} and he steals the HBT title outright. Yes, unfortunately, Corey actually controls his own destiny.`;
  }
  if(p.id==='am'){
    return `Anthony is ${fmt(gap)} points back, but a five-win season has kept him dangerous all year. He does not need a favor, a miracle, or a spreadsheet full of scenarios: ${guaranteeText} and he leaves the season with the HBT title. Clean, difficult, and completely in his hands.`;
  }
  return `${p.first} controls his own destiny in the HBT title race. ${gap===0?`${p.first} leads the standings.`:`${p.first} is ${fmt(gap)} points behind ${leaderName}.`} The guaranteed path is to ${guaranteeText}. Do that and the title is his regardless of every other result.`;
}

function titleRaceNeedsHelpBlurb(p,gap){
  if(p.id==='rg'){
    return `Rick picked the perfect time to blow the title race back open with his R15 win. He is ${fmt(gap)} points behind the lead, and even a sweep of ${titleRaceActionText()} still requires the guys ahead of him to bleed some points. But if Tony, Corey, and Anthony crack the door, Rick is close enough to kick it in.`;
  }
  if(p.id==='ss'){
    return `Scott is ${fmt(gap)} points back, so we have officially reached chaos-scenario territory. He needs a heater at ${titleRaceActionText()} and then needs the top of the leaderboard to turn into a small disaster. The path is ugly, the odds are long, but the math has not killed it yet.`;
  }
  return `${p.first} is ${fmt(gap)} points back and still mathematically alive, but the title is no longer completely in his hands. Even winning ${titleRaceActionText()} would require help from the leaders along the way.`;
}

function titleRaceEliminatedBlurb(p){
  if(p.id==='jb'){
    return `Josh's 2026 title path has officially closed, and missing R15 made an already steep points climb impossible. That is a rough way for the race to end, but there are still meaningful chances left to finish the season well. R16 and the HBT Championship are two shots to grab a big result, play spoiler, and carry some momentum into next year.`;
  }
  return `${p.first}'s run at the 2026 HBT title is officially over. Even winning ${titleRaceActionText()} would max him out at ${fmt(titleRaceMaxPoints(p.id))} points, so the championship math no longer works — but there is still plenty left to play for before the season ends.`;
}
  ...
  standings.forEach(entry=>{
    ...
    if(titleClinched(p.id)){
      map[p.id]={
        status:'clinched',
        label:'CLINCHED',
        blurb:`${p.first} has done it — the title race is officially over. The remaining results cannot change the math, and the 2026 HBT season championship belongs to ${p.first}.`
      };
      return;
    }
    ...
    if(guarantee){
      ...
      map[p.id]={
        ...
        blurb:titleRaceControlsBlurb(p,gap,leader,guaranteeText)
      };
      return;
    }
    ...
    if(alive){
      map[p.id]={
        status:'needs-help',
        label:'NEEDS HELP',
        blurb:titleRaceNeedsHelpBlurb(p,gap)
      };
      return;
    }
    ...
    map[p.id]={
      status:'eliminated',
      label:'ELIMINATED',
      blurb:titleRaceEliminatedBlurb(p)
    };
  });
...
function home(){
  ...
  const latestRoundId=latestScorecard?.roundId||D.latestRound?.id||'R15';
  ...
  const fcfg=seasonConfig();
  const fullPointOrder=[...seasonRegularRoundIds().slice(0,10),fcfg.mc.id,...seasonRegularRoundIds().slice(10),fcfg.ch.id];
  const futurePointRows=fullPointOrder.filter(r=>!labs.includes(r));
  ...
  const totalSeasonEvents=seasonConfig().regularRounds+2;
  const roundsRemaining=Math.max(totalSeasonEvents-roundsPlayed,0);
  ...
  const roundDateMap={
    ...
    R13:'July 21, 2026',R14:'August 4, 2026',R15:'August 11, 2026'
  };
  if(latestScorecard?.roundId && latestScorecard?.course){
    roundCourseMap[latestScorecard.roundId]=String(latestScorecard.course).toUpperCase();
  }
  if(latestScorecard?.roundId && latestScorecard?.date){
    const d=new Date(latestScorecard.date);
    if(!Number.isNaN(d.getTime())) roundDateMap[latestScorecard.roundId]=d.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
  }
  ...
  const latestRoundRating=Number.isFinite(Number(latestScorecard?.rating))?Number(latestScorecard.rating):(typeof latestRoundCourse?.nineRating==='number'?latestRoundCourse.nineRating:null);
...
<div class="hero-video-treatment" aria-hidden="true"></div>
    ...
    <div class="hero-in home-hero-in">
      ...
      <div class="hero-status hero-status-clean">
        <span>${roundsPlayed} of ${totalSeasonEvents} events complete</span>
  ...
  const render2026=()=>`
    <div class="rounds-list scorecards-list">
      ${completed.map(s=>{
        ...
        const courseName=(D.latestScorecard?.roundId===s.id && D.latestScorecard?.course)
          ? String(D.latestScorecard.course).toUpperCase()
          : (courseByRound[s.id]||s.id);
        ...
        const isLatestScorecard=D.latestScorecard?.roundId===s.id;
        const rating=isLatestScorecard && Number.isFinite(Number(D.latestScorecard?.rating))
          ? Number(D.latestScorecard.rating)
          : (course?(is18?course.rating:course.nineRating):null);
        const slope=isLatestScorecard?(D.latestScorecard?.slope??course?.slope):course?.slope;
        const tees=isLatestScorecard?String(D.latestScorecard?.tees||course?.tees||'').toUpperCase():(course?.tees||'—');
            ...
            <div class="round-scorecard-header-top">
              <div class="round-scorecard-title-block">
                ...
                <div class="round-course-meta">
                  ...
                  <span>${typeof rating==='number'?rating.toFixed(2):'—'} | ${slope??'—'}</span>
                  <span>PAR ${par??'—'}</span>
                </div>
              </div>
...
const PLAYER_2026_ROUND_ORDER=[
  ...seasonRegularRoundIds().slice(0,10),
  seasonConfig().mc.id,
  ...seasonRegularRoundIds().slice(10),
  seasonConfig().ch.id
];
