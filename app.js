...
function home(){
  ...
  const futurePointRows=seasonPointRoundOrder().filter(r=>!pointRoundComplete(r));
...
<div class="hero-video-treatment" aria-hidden="true"></div>
  ...
  <section class="page home-main">
    ...
    <section class="premium-section reveal">
      ...
      <p class="premium-copy">Lowest 2 regular-season scores are dropped. MC always counts. The HBT Championship has three separate scoring results—Front 9, Back 9 and Major Result—and all three count.</p>
      <div class="tablebox home-points compact-points" style="margin-top:22px">
        <table>
          ...
          <tbody>
            ${labs.map(r=>`<tr><td class="pname">${pointRoundDisplayLabel(r)}</td>${D.players.map(p=>{
              const drops=droppedPointRounds(p.id,labs);
              const val=D.pointsByRound[r]?.[p.id];
              const cls=drops.has(r)?'dropped-point':'';
              return `<td class="${cls}">${val??'—'}</td>`;
            }).join('')}</tr>`).join('')}
            ${futurePointRows.map(r=>`<tr class="future-points-row"><td class="pname">${pointRoundDisplayLabel(r)}</td>${D.players.map(()=>'<td>—</td>').join('')}</tr>`).join('')}
...
function leaderboard(){
  ...
  ${labs.map(r=>`<tr><td class="pname">${r}</td>${playerOrder.map(id=>`<td>${D.pointsByRound[r]?.[id]??''}</td>`).join('')}</tr>`).join('')}
