  ...
  return `<section class="page records-page">
    <div class="pagehead stats-pagehead">
      <div>
        ...
        <p class="records-page-note">All-time HBT records and personal bests from the league handicap history.</p>
      </div>
    </div>

    <div class="coursegrid records-grid">
      ${D.courses.map(c=>`<article class="card statcard record-course-card reveal">
        ...
        ${c.personal.slice(0,6).map((x,i)=>{
          ...
          return `<div class="statrow record-statrow ${isRecord?'statrow-leader':''}">
            <div class="rank">${i+1}</div>
            ${avatar(p)}
            <div class="stat-player-name"><b>${p.last}</b>${isRecord?'<div class="sub record-holder-note">Course record</div>':''}</div>
