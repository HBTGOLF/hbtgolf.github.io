...
function initHeroIntro(){
  ...
  // Logo fades in exactly where it will remain.
  setTimeout(()=>hero.classList.add('hero-logo-visible'),300);

  // Black screen fades away, revealing the homepage under the stationary logo.
  setTimeout(()=>intro.classList.add('intro-fading'),1350);

  // Supporting UI fades in after the page is visible.
  setTimeout(()=>{
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initPremiumMotion();initHeroStatusRotation();initHeroIntro();initMenu();});
};
