...
function hardestCoursesTable(){
  ...
  return `<section class="card league-history-card reveal">
    <div class="pad league-history-card-head">
      ...
      <p class="caption">Regular HBT 9-hole courses ranked by average gross score across all recorded HC rounds.</p>
    </div>
    <div class="tablebox league-history-tablebox">
      <table class="league-history-table hardest-course-table">
        <colgroup><col class="hc-rank-col"><col class="hc-course-col"><col class="hc-avg-col"><col class="hc-rounds-col"></colgroup>
        <thead><tr><th>#</th><th>Course</th><th>Avg Gross</th><th>Rounds</th></tr></thead>
        <tbody>
          ${rows.map((r,i)=>`<tr class="${i===0?'history-leader-row':''}"><td>${r.rank}</td><td class="pname history-course">${r.course}</td><td class="history-feature-value">${Number(r.avg).toFixed(2)}</td><td>${r.rounds}</td></tr>`).join('')}
        </tbody>
      </table>
    </div>
  </section>`;
...
function stats(){
  ...
  const leagueStats=`<div class="league-history-stack">
    ${leagueRoundHistoryTable(
      'Best Rounds All Time',
      'The 10 lowest scoring differentials recorded in HBT history.',
    ...
    ${leagueRoundHistoryTable(
      ...
      'The 10 lowest gross scores from 9-hole rounds only.',
    ...
    ${leagueRoundHistoryTable(
      ...
      'The 10 highest gross scores from 9-hole rounds only.',
  ...
  return `<section class="page stats-page">
    <div class="pagehead stats-pagehead">
      <div><div class="eyebrow">League Analytics</div><h1 id="statsPageTitle">2026 Stats</h1></div>
    </div>

    <div class="stats-view-toggle" role="tablist" aria-label="Stats view">
      <button type="button" class="stats-view-button active" data-stats-view="season" role="tab" aria-selected="true">2026 Season</button>
...
function initStatsViewToggle(){
  ...
  const show=view=>{
    ...
    if(title) title.textContent=view==='league'?'League Stats':'2026 Stats';
