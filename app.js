...
function rounds(){
  ...
  return `<section class="page scorecards-page">
    ...
    <div class="scorecard-year-wrap" aria-label="Scorecard seasons">
      <div class="scorecard-year-nav">
        ${years.map(y=>`
          <button class="scorecard-year ${y===2026?'active':'disabled'}"
                  type="button"
                  ${y===2026?'aria-current="page"':'disabled aria-disabled="true"'}
                  aria-label="${y}${y===2026?' scorecards':' scorecards unavailable'}">
            <span>${y}</span>
            ${y!==2026?'<small>Coming soon</small>':'<small>Current</small>'}
          </button>`).join('')}
      </div>
    </div>
