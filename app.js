  ...
  return `<section class="hero home-editorial-hero">
    ...
    <div class="hero-in home-hero-in">
      <div class="hero-title-wrap hero-logo-lockup">
        <img src="assets/hbt-logo.png" alt="HBT Golf League" class="hero-logo-clean">
        <div class="hero-season-overline">2026 Season</div>
      </div>
      <div class="hero-score-divider" aria-hidden="true"></div>
      <div class="hero-status hero-status-clean">
        <span>Through Round 14</span>
...
function initHeroIntro(){
  ...
  const introLogo=document.getElementById('siteIntroLogo');
  const hero=document.querySelector('.home-editorial-hero');
  const finalLogo=document.querySelector('.hero-logo-clean');

  if(!intro || !introLogo || !hero || !finalLogo) return;

  if(window.__hbtHeroIntroPlayed){
    intro.classList.add('intro-hidden');
    hero.classList.add('hero-intro-complete','hero-ui-visible');
    document.body.classList.add('site-ready');
    return;
  }
  window.__hbtHeroIntroPlayed=true;
  ...
  // Measure the FINAL logo and put the intro copy at those exact viewport coordinates.
  ...
  // The real logo is already sitting beneath the black overlay in the same spot.
  finalLogo.style.opacity='1';

  // Plain black hold, then simple logo fade.
  setTimeout(()=>intro.classList.add('intro-logo-visible'),350);

  // Hold the white logo briefly.
  setTimeout(()=>intro.classList.add('intro-fading'),1550);

  // Fade the supporting UI in while the black veil finishes disappearing.
  setTimeout(()=>{
    hero.classList.add('hero-ui-visible');
    document.body.classList.add('site-ready');
  },1850);
...
render=function(){
  baseRender();
  requestAnimationFrame(()=>{initPremiumMotion();initHeroStatusRotation();initHeroIntro();setupMenu();});
};
