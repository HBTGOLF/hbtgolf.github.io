...
function canWinTitleIfSweeps(playerId){
  ...
  const remainingEvents=[
    ...
    ...rem.ch.map(card=>({id:card.id,type:'ch',points:card.points}))
  ];
  ...
  // With more than three scoring cards left, use a conservative test that
  // cannot falsely eliminate a player. Once three or fewer remain, run the
  // correlated finish-assignment search below.
  ...
  // Prefer a non-droppable scoring card as the final matching step.
  ...
  const assignedSpecial=Object.fromEntries(rivals.map(r=>[r.id,{}]));

  const assignEvent=(event,perm)=>{
    rivals.forEach((r,i)=>{
      const pts=event.points[perm[i]];
      if(event.type==='regular') assignedRegular[r.id].push(pts);
      else assignedSpecial[r.id][event.id]=pts;
    });
  };
  const unassignEvent=(event)=>{
    rivals.forEach(r=>{
      if(event.type==='regular') assignedRegular[r.id].pop();
      else delete assignedSpecial[r.id][event.id];
    });
  };

  const rivalBase=(r)=>{
    const special=assignedSpecial[r.id];
    return finalSeasonPoints(r.id,{
      regularFuture:assignedRegular[r.id],
      mcFuture:special[f.mc.id]??null,
      chFuture:Object.fromEntries(f.ch.cards
        .filter(card=>special[card.id]!==undefined)
        .map(card=>[card.id,special[card.id]]))
    });
  };
...
function home(){
  ...
  const latestRoundId=completedSeasonPointRounds().at(-1) || D.latestRound?.id || 'R1';
  ...
  const latestCourseName=latestMeta.course||roundCourseMap[latestRoundId]||D.latestRound?.course;
...
<div class="hero-video-treatment" aria-hidden="true"></div>
  ...
  <section class="page home-main">
    ...
    <section class="premium-section reveal">
      ...
      <p class="premium-copy">Lowest 2 regular-season scores are dropped. MC always counts. CH1, CH2 and CH3 each use standard points and all three count.</p>
...
function leaderboard(){
  ...
  ${currentSeasonStandings().map((entry,i)=>{const p=entry.player;return `<tr class="${i===0?'leadrow':''}"><td class="pos">${entry.pos}</td><td><div style="display:flex;align-items:center;gap:10px">${avatar(p)}<div><div class="pname">${p.last}</div><div class="sub">${p.initials}</div></div></div></td><td>${p.hcp.toFixed(2)}${hcpArrow(p)}</td><td>${playerSeason2026FromRounds(p.id).wins}</td><td class="pts">${fmt(entry.points)}</td><td>${i?'−'+fmt(currentSeasonStandings()[0].points-entry.points):'—'}</td></tr>`}).join('')}
  </tbody></table></div>
...
function rounds(){
  const summaryById=Object.fromEntries((D.roundSummaries||[]).map(s=>[s.id,s]));
  const completed=seasonPointOrder()
    .filter(id=>(D.rounds?.[id]||[]).some(x=>x.gross!=null))
    .map(id=>summaryById[id]||{id,name:id})
    .reverse();
...
function playerSeason2026FromRounds(playerId){
  const results=player2026RoundOrder().map(round=>{
    ...
    return {
      ...
      points:roundPoints(pointsKey,playerId)??0
    };
  }).filter(Boolean);
